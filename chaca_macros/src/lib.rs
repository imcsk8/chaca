...
/// Structure for federal candidate lists
/// TODO: generalize to have only one macro for candidate details
#[macro_export]
macro_rules! federal_candidate_details {
    () => {
            use diesel::sql_types::*;
            use serde::{Serialize, Deserialize};
            use diesel::serialize::ToSql;
            ...
            // Define a structure to hold the joined result
            #[derive(Debug, QueryableByName, Serialize)]
            struct FederalCandidateWithDetails {
                #[diesel(sql_type = Uuid)]
                id: uuid::Uuid,
                #[diesel(sql_type = Nullable<Uuid>)]
                external_uuid: Option<uuid::Uuid>,
...
/// Macro that generates query for federal positions by state and INE section
/// Arguments
/// position: Position id for cat_positions
#[macro_export]
macro_rules! federal_position_query_by_sate_section {
    ($position:expr) => {
        format!("SELECT c.*,
            CASE
                WHEN c.sex = 'HOMBRE' THEN concat_ws(' ', p.male_name, p.long_name)
        ...
        FROM candidate c
        JOIN cat_state s ON c.state = s.id_inegi
        JOIN cat_estado_circuito ca ON c.state = ca.idestado
        JOIN cat_positions p ON c.position = p.id
        JOIN cat_poder po ON c.poder = po.uuid
        LEFT JOIN cat_matter m ON c.matter = m.uuid
        LEFT JOIN LATERAL (
        ...
        WHERE c.is_federal = true AND c.position={} AND c.state=$1 AND ca.seccion=$2
        ORDER BY c.fullname", $position)
    };
}
...
/// Macro that generates query for state candidates
#[macro_export]
macro_rules! list_by_state_query {
    () => {
        format!("SELECT c.*,
            CASE
                WHEN c.sex = 'HOMBRE' THEN concat_ws(' ', p.male_name, p.long_name)
        ...
        WHERE c.state = $1 AND is_federal = false
        ORDER BY c.fullname")
    };
}


/// Macro that generates the query for federal candidates
#[macro_export]
macro_rules! list_all_federal_query {
    () => {
        format!("SELECT c.*,
            CASE
                WHEN c.sex = 'HOMBRE' THEN concat_ws(' ', p.male_name, p.long_name)
...
/// Candidate profile quiery
#[macro_export]
macro_rules! candidate_profile_query {
    () => {
        String::from(
            "SELECT c.*,
                CASE
